...
transaction() {
    prepare(deployer: AuthAccount) {
        ...
        let pairAddress = SwapFactory2.getPairAddress(token0Key: token0Key, token1Key: token1Key)
        log("pair address: ")
        log(pairAddress)
        if (pairAddress == nil) {
            ...
            SwapFactory2.createPair(token0Vault: <-token0Vault, token1Vault: <-token1Vault, accountCreationFee: <-fee)
        } else {
            /// Pair already exists
            destroy token0Vault
            destroy token1Vault
