...
transaction() {
    prepare(deployer: AuthAccount) {
        ...
        let pairAddress = SwapFactory1.getPairAddress(token0Key: token0Key, token1Key: token1Key)
        log("pair address: ")
        log(pairAddress)
        if (pairAddress == nil) {
            ...
            SwapFactory1.createPair(token0Vault: <-token0Vault, token1Vault: <-token1Vault, accountCreationFee: <-fee)
        } else {
            /// Pair already exists
            destroy token0Vault
            destroy token1Vault
