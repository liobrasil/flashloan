...
transaction(
    ...
    prepare(lp: AuthAccount) {
        let pairAddr = SwapFactory2.getPairAddress(token0Key: token0Key, token1Key: token1Key)
        ...
        */
        ...
        var lpTokenCollectionRef = lp.borrow<&SwapFactory2.LpTokenCollection>(from: lpTokenCollectionStoragePath)
        ...
        if lpTokenCollectionRef == nil {
            ...
            lp.save(<-SwapFactory2.createEmptyLpTokenCollection(), to: lpTokenCollectionStoragePath)
            lp.link<&{SwapInterfaces.LpTokenCollectionPublic}>(lpTokenCollectionPublicPath, target: lpTokenCollectionStoragePath)
            lpTokenCollectionRef = lp.borrow<&SwapFactory2.LpTokenCollection>(from: lpTokenCollectionStoragePath)
        }
